...
static void _RRM_reset_analog_data(void) {
    ...
    // VIN / VOUT.
    SWREG_write_field(&reg_analog_data_1, &reg_analog_data_1_mask, UNA_VOLTAGE_ERROR_VALUE, RRM_REGISTER_ANALOG_DATA_1_MASK_VIN);
    SWREG_write_field(&reg_analog_data_1, &reg_analog_data_1_mask, UNA_VOLTAGE_ERROR_VALUE, RRM_REGISTER_ANALOG_DATA_1_MASK_VOUT);
    ...
    // IOUT.
    SWREG_write_field(&reg_analog_data_2, &reg_analog_data_2_mask, UNA_VOLTAGE_ERROR_VALUE, RRM_REGISTER_ANALOG_DATA_2_MASK_IOUT);
...
NODE_status_t RRM_check_register(uint8_t reg_addr, uint32_t reg_mask) {
    ...
    case RRM_REGISTER_ADDRESS_CONTROL_1:
        ...
        // ZCCT.
        if ((reg_mask & RRM_REGISTER_CONTROL_1_MASK_ZCCT) != 0) {
            // Read bit.
            if (SWREG_read_field(reg_value, RRM_REGISTER_CONTROL_1_MASK_ZCCT) != 0) {
                // Clear request.
                NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, RRM_REGISTER_ADDRESS_CONTROL_1, 0b0, RRM_REGISTER_CONTROL_1_MASK_ZCCT);
                // Turn analog front-end on.
                POWER_enable(POWER_REQUESTER_ID_RRM, POWER_DOMAIN_ANALOG, LPTIM_DELAY_MODE_ACTIVE);
                // Get output current.
                analog_status = ANALOG_convert_channel(ANALOG_CHANNEL_IOUT_UA, &output_current_ua);
                ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
                // Write register and NVM.
                SWREG_write_field(&reg_config_1, &reg_config_1_mask, UNA_convert_ua(output_current_ua), RRM_REGISTER_CONFIGURATION_1_MASK_IOUT_OFFSET);
                NODE_write_register(NODE_REQUEST_SOURCE_EXTERNAL, RRM_REGISTER_ADDRESS_CONFIGURATION_1, reg_config_1, reg_config_1_mask);
            }
        }
        break;
    default:
...
NODE_status_t RRM_mtrg_callback(void) {
    ...
    ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
    SWREG_write_field(&reg_analog_data_1, &reg_analog_data_1_mask, (uint32_t) UNA_convert_mv(adc_data), RRM_REGISTER_ANALOG_DATA_1_MASK_VIN);
    // Regulator output voltage.
    analog_status = ANALOG_convert_channel(ANALOG_CHANNEL_VOUT_MV, &adc_data);
    ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
    SWREG_write_field(&reg_analog_data_1, &reg_analog_data_1_mask, (uint32_t) UNA_convert_mv(adc_data), RRM_REGISTER_ANALOG_DATA_1_MASK_VOUT);
    vsh_mv = adc_data;
    ...
    if (vsh_mv >= RRM_IOUT_MEASUREMENT_VSH_MIN_MV) {
        ...
        ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
        // Read IOUT offset.
        NODE_read_register(NODE_REQUEST_SOURCE_INTERNAL, RRM_REGISTER_ADDRESS_CONFIGURATION_1, &reg_config_1);
        lt6106_offset_current_ua = UNA_get_ua(SWREG_read_field(reg_config_1, RRM_REGISTER_CONFIGURATION_1_MASK_IOUT_OFFSET));
        // Remove offset.
        adc_data = (adc_data < lt6106_offset_current_ua) ? 0 : (adc_data - lt6106_offset_current_ua);
        SWREG_write_field(&reg_analog_data_2, &reg_analog_data_2_mask, (uint32_t) UNA_convert_ua(adc_data), RRM_REGISTER_ANALOG_DATA_2_MASK_IOUT);
    }
    // Write registers.
    NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, RRM_REGISTER_ADDRESS_ANALOG_DATA_1, reg_analog_data_1, reg_analog_data_1_mask);
    NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, RRM_REGISTER_ADDRESS_ANALOG_DATA_2, reg_analog_data_2, reg_analog_data_2_mask);
