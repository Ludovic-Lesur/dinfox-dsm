...
#define BPSM_XVF_UPDATE_PERIOD_SECONDS              (2 * MATH_SECONDS_PER_MINUTE)

#ifdef BPSM_CHARGE_CONTROL_FORCED_HARDWARE
#define BPSM_FLAG_CCFH                              0b1
#else
#define BPSM_FLAG_CCFH                              0b0
#endif
#ifdef BPSM_CHARGE_STATUS_FORCED_HARDWARE
#define BPSM_FLAG_CHST                              0b1
#else
#define BPSM_FLAG_CHST                              0b0
#endif
#ifdef BPSM_BACKUP_CONTROL_FORCED_HARDWARE
...
#ifndef BPSM_CHARGE_CONTROL_FORCED_HARDWARE
    bpsm_ctx.source_voltage_mv = 0;
    bpsm_ctx.charge_toggle_previous_time_seconds = 0;
    bpsm_ctx.charge_toggle_next_time_seconds = 0;
#endif
    return status;
}
...
void BPSM_init_register(uint8_t reg_addr, uint32_t* reg_value) {
    ...
    case BPSM_REGISTER_ADDRESS_CONFIGURATION_0:
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_seconds(BPSM_CHARGE_TOGGLE_PERIOD_SECONDS), BPSM_REGISTER_CONFIGURATION_0_MASK_CHARGE_TOGGLE_PERIOD);
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_mv(BPSM_CHARGE_SOURCE_VOLTAGE_TH_MV), BPSM_REGISTER_CONFIGURATION_0_MASK_CHARGE_SOURCE_VOLTAGE_TH);
        break;
    case BPSM_REGISTER_ADDRESS_CONFIGURATION_1:
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_mv(BPSM_LVF_STORAGE_VOLTAGE_THL_MV), BPSM_REGISTER_CONFIGURATION_1_MASK_LVF_STORAGE_VOLTAGE_THL);
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_mv(BPSM_LVF_STORAGE_VOLTAGE_THH_MV), BPSM_REGISTER_CONFIGURATION_1_MASK_LVF_STORAGE_VOLTAGE_THH);
        break;
    case BPSM_REGISTER_ADDRESS_CONFIGURATION_2:
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_mv(BPSM_CVF_STORAGE_VOLTAGE_THL_MV), BPSM_REGISTER_CONFIGURATION_2_MASK_CVF_STORAGE_VOLTAGE_THL);
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_mv(BPSM_CVF_STORAGE_VOLTAGE_THH_MV), BPSM_REGISTER_CONFIGURATION_2_MASK_CVF_STORAGE_VOLTAGE_THH);
        break;
#endif
    default:
        break;
...
NODE_status_t BPSM_secure_register(uint8_t reg_addr, uint32_t new_reg_value, uint32_t* reg_mask, uint32_t* reg_value) {
    ...
    case BPSM_REGISTER_ADDRESS_CONFIGURATION_0:
        SWREG_secure_field(
            ...
            UNA_get_mv,
            UNA_convert_mv,
            > BPSM_CHARGE_SOURCE_VOLTAGE_TH_MV_MAX,
            > BPSM_CHARGE_SOURCE_VOLTAGE_TH_MV_MAX,
            BPSM_CHARGE_SOURCE_VOLTAGE_TH_MV_DEFAULT,
        ...
        SWREG_secure_field(
            ...
            UNA_get_seconds,
            UNA_convert_seconds,
            < BPSM_CHARGE_TOGGLE_PERIOD_SECONDS_MIN,
            > BPSM_CHARGE_TOGGLE_PERIOD_SECONDS_MAX,
            BPSM_CHARGE_TOGGLE_PERIOD_SECONDS_DEFAULT,
            status = NODE_ERROR_REGISTER_FIELD_VALUE
        );
        break;
    case BPSM_REGISTER_ADDRESS_CONFIGURATION_1:
        ...
        SWREG_secure_field(
            ...
            UNA_get_mv,
            UNA_convert_mv,
            > BPSM_XVF_STORAGE_VOLTAGE_TH_MV_MAX,
            > high_threshold_mv,
            0,
            status = NODE_ERROR_REGISTER_FIELD_VALUE
        );
...
NODE_status_t BPSM_process_register(uint8_t reg_addr, uint32_t reg_mask) {
    ...
    NODE_status_t status = NODE_SUCCESS;
    uint32_t* reg_ptr = &(NODE_RAM_REGISTER[reg_addr]);
#ifndef BPSM_BACKUP_CONTROL_FORCED_HARDWARE
...
NODE_status_t BPSM_low_voltage_detector_process(void) {
    ...
    uint32_t* reg_config_1_ptr = &(NODE_RAM_REGISTER[BPSM_REGISTER_ADDRESS_CONFIGURATION_1]);
    uint32_t* reg_config_2_ptr = &(NODE_RAM_REGISTER[BPSM_REGISTER_ADDRESS_CONFIGURATION_2]);
    ...
    if (uptime_seconds >= bpsm_ctx.lvf_cvf_update_next_time_seconds) {
        ...
        ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
#ifndef BPSM_CHARGE_CONTROL_FORCED_HARDWARE
        analog_status = ANALOG_convert_channel(ANALOG_CHANNEL_SOURCE_VOLTAGE_MV, &(bpsm_ctx.source_voltage_mv));
        ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
#endif
        // Update LVF flag.
        if (storage_voltage_mv < UNA_get_mv(SWREG_read_field((*reg_config_1_ptr), BPSM_REGISTER_CONFIGURATION_1_MASK_LVF_STORAGE_VOLTAGE_THL))) {
            SWREG_write_field(reg_status_1_ptr, &unused_mask, 0b1, BPSM_REGISTER_STATUS_1_MASK_LVF);
        }
        if (storage_voltage_mv > UNA_get_mv(SWREG_read_field((*reg_config_1_ptr), BPSM_REGISTER_CONFIGURATION_1_MASK_LVF_STORAGE_VOLTAGE_THH))) {
            SWREG_write_field(reg_status_1_ptr, &unused_mask, 0b0, BPSM_REGISTER_STATUS_1_MASK_LVF);
        }
        // Update CVF flag.
        if (storage_voltage_mv < UNA_get_mv(SWREG_read_field((*reg_config_2_ptr), BPSM_REGISTER_CONFIGURATION_2_MASK_CVF_STORAGE_VOLTAGE_THL))) {
            SWREG_write_field(reg_status_1_ptr, &unused_mask, 0b1, BPSM_REGISTER_STATUS_1_MASK_CVF);
        }
        if (storage_voltage_mv > UNA_get_mv(SWREG_read_field((*reg_config_2_ptr), BPSM_REGISTER_CONFIGURATION_2_MASK_CVF_STORAGE_VOLTAGE_THH))) {
...
NODE_status_t BPSM_charge_process(void) {
    ...
    // Check toggle period.
    if (uptime_seconds >= bpsm_ctx.charge_toggle_next_time_seconds) {
        // Update times.
        bpsm_ctx.charge_toggle_previous_time_seconds = uptime_seconds;
        bpsm_ctx.charge_toggle_next_time_seconds = uptime_seconds + ((uint32_t) UNA_get_seconds(SWREG_read_field(reg_config_0, BPSM_REGISTER_CONFIGURATION_0_MASK_CHARGE_TOGGLE_PERIOD)));
        // Disable charge.
        LOAD_set_charge_state(0);
    }
    if (uptime_seconds >= (bpsm_ctx.charge_toggle_previous_time_seconds + BPSM_CHARGE_TOGGLE_DURATION_SECONDS)) {
        // Check voltage.
        if (bpsm_ctx.source_voltage_mv >= UNA_get_mv(SWREG_read_field(reg_config_0, BPSM_REGISTER_CONFIGURATION_0_MASK_CHARGE_SOURCE_VOLTAGE_TH))) {
            // Enable charge.
