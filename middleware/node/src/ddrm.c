...
NODE_status_t DDRM_init(void) {
    ...
    // Init context.
    ddrm_ctx.regulator_control_state = UNA_BIT_ERROR;
    return status;
}
...
void DDRM_init_register(uint8_t reg_addr, uint32_t* reg_value) {
    ...
    case DDRM_REGISTER_ADDRESS_FLAGS_1:
        SWREG_write_field(reg_value, &unused_mask, DDRM_FLAG_RCFH, DDRM_REGISTER_FLAGS_1_MASK_RCFH);
        break;
#ifdef DSM_NVM_FACTORY_RESET
    case DDRM_REGISTER_ADDRESS_CONFIGURATION_0:
        SWREG_write_field(reg_value, &unused_mask, UNA_convert_ua(DDRM_OUTPUT_CURRENT_OFFSET_UA_DEFAULT), DDRM_REGISTER_CONFIGURATION_0_MASK_OUTPUT_CURRENT_OFFSET);
        break;
#endif
    default:
        break;
...
void DDRM_refresh_register(uint8_t reg_addr) {
    ...
    case DDRM_REGISTER_ADDRESS_STATUS_1:
        // DC-DC state.
#ifdef DDRM_REGULATOR_CONTROL_FORCED_HARDWARE
        ddrm_ctx.regulator_control_state = UNA_BIT_FORCED_HARDWARE;
#else
        switch (LOAD_get_output_state()) {
        case 0:
            ddrm_ctx.regulator_control_state = UNA_BIT_0;
            break;
        case 1:
            ddrm_ctx.regulator_control_state = UNA_BIT_1;
            break;
        default:
            ddrm_ctx.regulator_control_state = UNA_BIT_ERROR;
            break;
...
#endif
        SWREG_write_field(reg_ptr, &unused_mask, ((uint32_t) ddrm_ctx.regulator_control_state), DDRM_REGISTER_STATUS_1_MASK_RCS);
        break;
    default:
        break;
    }
...
NODE_status_t DDRM_secure_register(uint8_t reg_addr, uint32_t new_reg_value, uint32_t* reg_mask, uint32_t* reg_value) {
    ...
    case DDRM_REGISTER_ADDRESS_CONFIGURATION_0:
        SWREG_secure_field(
            DDRM_REGISTER_CONFIGURATION_0_MASK_OUTPUT_CURRENT_OFFSET,
            UNA_get_ua,
            UNA_convert_ua,
            > DDRM_OUTPUT_CURRENT_OFFSET_UA_MAX,
            > DDRM_OUTPUT_CURRENT_OFFSET_UA_MAX,
            DDRM_OUTPUT_CURRENT_OFFSET_UA_DEFAULT,
...
NODE_status_t DDRM_process_register(uint8_t reg_addr, uint32_t reg_mask) {
    ...
    NODE_status_t status = NODE_SUCCESS;
    uint32_t* reg_ptr = &(NODE_RAM_REGISTER[reg_addr]);
#ifndef DDRM_REGULATOR_CONTROL_FORCED_HARDWARE
    ...
    case DDRM_REGISTER_ADDRESS_CONTROL_1:
        // DDEN.
        if ((reg_mask & DDRM_REGISTER_CONTROL_1_MASK_REN) != 0) {
            // Check pin mode.
#ifdef DDRM_REGULATOR_CONTROL_FORCED_HARDWARE
...
#else
            // Read bit.
            dden = SWREG_read_field((*reg_ptr), DDRM_REGISTER_CONTROL_1_MASK_REN);
            // Compare to current state.
            if (dden != ddrm_ctx.regulator_control_state) {
                // Set DC-DC state.
...
NODE_status_t DDRM_mtrg_callback(void) {
    ...
    SWREG_write_field(reg_analog_data_1_ptr, &unused_mask, UNA_convert_mv(input_voltage_mv), DDRM_REGISTER_ANALOG_DATA_1_MASK_INPUT_VOLTAGE);
    // DC-DC output voltage.
    analog_status = ANALOG_convert_channel(ANALOG_CHANNEL_OUTPUT_VOLTAGE_MV, &output_voltage_mv);
    ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
    SWREG_write_field(reg_analog_data_1_ptr, &unused_mask, UNA_convert_mv(output_voltage_mv), DDRM_REGISTER_ANALOG_DATA_1_MASK_OUTPUT_VOLTAGE);
    // Check output current measurement validity.
    if (output_voltage_mv >= DDRM_OUTPUT_CURRENT_MEASUREMENT_POWER_TH_MV) {
        // DC-DC output current.
        analog_status = ANALOG_convert_channel(ANALOG_CHANNEL_OUTPUT_CURRENT_UA, &output_current_ua);
        ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
    }
    SWREG_write_field(reg_analog_data_2_ptr, &unused_mask, UNA_convert_ua(output_current_ua), DDRM_REGISTER_ANALOG_DATA_2_MASK_OUTPUT_CURRENT);
errors:
    return status;
}
