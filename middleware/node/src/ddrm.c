...
void DDRM_init_register(uint8_t reg_addr, uint32_t* reg_value) {
    ...
    case DDRM_REGISTER_ADDRESS_CONFIGURATION_0:
        SWREG_write_field(reg_value, &unused_mask, DDRM_IOUT_OFFSET_UA_DEFAULT, DDRM_REGISTER_CONFIGURATION_0_MASK_OUTPUT_CURRENT_OFFSET);
        break;
#endif
    default:
        break;
...
NODE_status_t DDRM_mtrg_callback(void) {
    ...
    SWREG_write_field(reg_analog_data_1_ptr, &unused_mask, UNA_convert_mv(vin_mv), DDRM_REGISTER_ANALOG_DATA_1_MASK_INPUT_VOLTAGE);
    // DC-DC output voltage.
    analog_status = ANALOG_convert_channel(ANALOG_CHANNEL_VOUT_MV, &vout_mv);
    ANALOG_exit_error(NODE_ERROR_BASE_ANALOG);
    SWREG_write_field(reg_analog_data_1_ptr, &unused_mask, UNA_convert_mv(vout_mv), DDRM_REGISTER_ANALOG_DATA_1_MASK_OUTPUT_VOLTAGE);
    ...
    SWREG_write_field(reg_analog_data_2_ptr, &unused_mask, UNA_convert_ua(iout_ua), DDRM_REGISTER_ANALOG_DATA_2_MASK_OUTPUT_CURRENT);
errors:
    return status;
}
