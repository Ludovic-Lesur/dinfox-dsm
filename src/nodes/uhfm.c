...
NODE_status_t UHFM_check_register(uint8_t reg_addr) {
	...
	case UHFM_REG_ADDR_STATUS_CONTROL_1:
		...
		// Read DTRG bit.
		if (DINFOX_read_field(reg_value, UHFM_REG_STATUS_CONTROL_1_MASK_DTRG) != 0) {
	...
	// Write register.
	node_status = NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, reg_addr, new_reg_mask, new_reg_value);
	NODE_stack_error();
errors:
	return status;
}
...
NODE_status_t UHFM_mtrg_callback(ADC_status_t* adc_status) {
	...
	NODE_stack_error();
	node_status = NODE_read_register(NODE_REQUEST_SOURCE_INTERNAL, UHFM_REG_ADDR_RADIO_TEST_1, &radio_test_1_initial);
	NODE_stack_error();
	...
	// Write registers.
	node_status = NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, UHFM_REG_ADDR_RADIO_TEST_0, radio_test_0, radio_test_0_mask);
	NODE_stack_error();
	node_status = NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, UHFM_REG_ADDR_RADIO_TEST_1, radio_test_1, radio_test_1_mask);
	NODE_stack_error();
	// Start CW.
	...
	NODE_stack_error();
	node_status = NODE_write_register(NODE_REQUEST_SOURCE_INTERNAL, UHFM_REG_ADDR_RADIO_TEST_1, DINFOX_REG_MASK_ALL, radio_test_1_initial);
	NODE_stack_error();
	return status;
}
#endif
