 ...
 */
static NODE_status_t _DDRM_reset_analog_data(void) {
	// Local variables.
	NODE_status_t status = NODE_SUCCESS;
	// Reset fields to error value.
	status = NODE_write_field(NODE_REQUEST_SOURCE_INTERNAL, DDRM_REG_ADDR_ANALOG_DATA_1, DDRM_REG_ANALOG_DATA_1_MASK_VIN, DINFOX_VOLTAGE_ERROR_VALUE);
	if (status != NODE_SUCCESS) goto errors;
	status = NODE_write_field(NODE_REQUEST_SOURCE_INTERNAL, DDRM_REG_ADDR_ANALOG_DATA_1, DDRM_REG_ANALOG_DATA_1_MASK_VOUT, DINFOX_VOLTAGE_ERROR_VALUE);
	if (status != NODE_SUCCESS) goto errors;
	status = NODE_write_field(NODE_REQUEST_SOURCE_INTERNAL, DDRM_REG_ADDR_ANALOG_DATA_2, DDRM_REG_ANALOG_DATA_2_MASK_IOUT, DINFOX_CURRENT_ERROR_VALUE);
	if (status != NODE_SUCCESS) goto errors;
errors:
	return status;
}
...
NODE_status_t DDRM_init_registers(void) {
	...
	if (status != NODE_SUCCESS) goto errors;
	// Load default values.
	status = _DDRM_reset_analog_data();
	if (status != NODE_SUCCESS) goto errors;
errors:
	return status;
}
...
NODE_status_t DDRM_mtrg_callback(void) {
	...
	ADC1_status_check(NODE_ERROR_BASE_ADC);
	status = NODE_write_field(NODE_REQUEST_SOURCE_INTERNAL, DDRM_REG_ADDR_ANALOG_DATA_1, DDRM_REG_ANALOG_DATA_1_MASK_VIN, (uint32_t) DINFOX_convert_mv(adc_data));
	if (status != NODE_SUCCESS) goto errors;
	// Relay output voltage.
	adc1_status = ADC1_get_data(ADC_DATA_INDEX_VOUT_MV, &adc_data);
	ADC1_status_check(NODE_ERROR_BASE_ADC);
	status = NODE_write_field(NODE_REQUEST_SOURCE_INTERNAL, DDRM_REG_ADDR_ANALOG_DATA_1, DDRM_REG_ANALOG_DATA_1_MASK_VOUT, (uint32_t) DINFOX_convert_mv(adc_data));
	if (status != NODE_SUCCESS) goto errors;
	// Relay output current.
	adc1_status = ADC1_get_data(ADC_DATA_INDEX_IOUT_UA, &adc_data);
	ADC1_status_check(NODE_ERROR_BASE_ADC);
	status = NODE_write_field(NODE_REQUEST_SOURCE_INTERNAL, DDRM_REG_ADDR_ANALOG_DATA_2, DDRM_REG_ANALOG_DATA_2_MASK_IOUT, (uint32_t) DINFOX_convert_ua(adc_data));
	if (status != NODE_SUCCESS) goto errors;
errors:
	return status;
}
